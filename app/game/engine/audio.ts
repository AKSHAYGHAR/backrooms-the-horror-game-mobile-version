...
export class GameAudio {
  ...
  setVolume(vol: number) {
    if (this.master) this.master.gain.value = vol * 0.9;
  }
  ...
  update(dt: number) {
    ...
    const silence = Math.max(0, 1 - p.dangerLevel * 1.5);

    ramp(this.hum.gain, (0.25 + p.humProximity * 0.75) * silence, 0.4);
    ramp(this.drone.gain, p.fear * 0.34 * silence, 0.8);
    ramp(this.chaseLayer.gain, (p.chasing ? 0.16 : 0) * silence, p.chasing ? 0.15 : 1.2);
    ramp(this.breathGain.gain, Math.max(0, p.fear - 0.45) * 0.1 * silence, 0.6);
