...
export function makeEntityMaps(seed: number): PBRMaps {
  ...
  for (let y = 0; y < S; y++) {
    for (let x = 0; x < S; x++) {
      const i = y * S + x;
      // Multi-octave veins at different scales for organic detail
      const veinsCoarse = n.fbm(x * 0.04, y * 0.04, 5);
      const veinsFine = n.fbm(x * 0.12, y * 0.12, 4);
      const veins = veinsCoarse * 0.7 + veinsFine * 0.3;
      const fine = n.noise(x * 0.8, y * 0.8);
      const pores = n2.noise(x * 2.5, y * 2.5); // micro skin pores

      // Pitch-black base with dark purple-red vein highlights
      const base = 6 + veins * 18 + fine * 5 + pores * 3;
      // Veins glow faintly reddish-purple in the crevices
      const veinGlow = Math.max(0, veins - 0.3) * 2.0;
      img.data[i * 4]     = Math.min(255, base * 1.1 + veinGlow * 22); // R — vein highlight
      img.data[i * 4 + 1] = Math.min(255, base * 0.85);                // G — suppressed
      img.data[i * 4 + 2] = Math.min(255, base * 0.82 + veinGlow * 8); // B — slight purple
      img.data[i * 4 + 3] = 255;

      height[i] = veinsCoarse * 0.6 + veinsFine * 0.25 + fine * 0.15;
      // Oily wet sheen: very low roughness in vein hollows, dryer on ridges
      const wetness = Math.max(0, veins - 0.2) * 0.6;
      rough[i] = 0.28 + veins * 0.25 - wetness + pores * 0.08;
    }
  }
  ctx.putImageData(img, 0, 0);
  return {
    map: tex(canvas, { srgb: true }),
    normalMap: tex(normalFromHeight(height, S, S, 3.0)), // stronger relief
    roughnessMap: tex(grayCanvas(rough, S, S)),
  };
}
