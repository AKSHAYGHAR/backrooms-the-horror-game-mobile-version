...
export class Player {
  ...
  touchMove = { x: 0, z: 0 };
  /** dedicated mobile sprint toggle / button */
  touchSprint = false;
  ...
  constructor(private level: Level, aspect: number) {
    ...
    this.flashlight = new THREE.SpotLight(0xfff3d6, isMobile ? 38 : 46, 28, 0.4, 0.5, 1.5);
    this.flashlight.position.set(0.18, -0.22, 0.05);
    // Disable flashlight shadow on mobile — single heaviest GPU cost
    this.flashlight.castShadow = !isMobile;
    this.flashlight.shadow.mapSize.set(isMobile ? 512 : 1024, isMobile ? 512 : 1024);
  ...
  onMouseDelta(dx: number, dy: number) {
    ...
    const sens = 0.0021 * this.mouseSens;
    this.yaw -= dx * sens;
    this.pitch -= (this.invertY ? -dy : dy) * sens;
    const lim = Math.PI / 2 - 0.06;
    this.pitch = Math.max(-lim, Math.min(lim, this.pitch));
  }

  /** Update camera aspect ratio & adjust FOV if in portrait to avoid tunnel vision */
  updateProjection(aspect: number) {
    this.camera.aspect = aspect;
    if (aspect < 1.0) {
      // In portrait mode, expand vertical FOV so horizontal visibility is not severely narrowed.
      // Standard 72 deg vertical FOV in 16:9 equates to ~100 deg horizontal FOV.
      // In portrait (< 1.0), calculate vertical FOV to maintain comfortable horizontal FOV:
      const targetHFov = (82 * Math.PI) / 180;
      const vFovRad = 2 * Math.atan(Math.tan(targetHFov / 2) / Math.max(0.4, aspect));
      this.camera.fov = Math.min(96, Math.max(72, (vFovRad * 180) / Math.PI));
    } else {
      this.camera.fov = 72;
    }
  ...
  update(dt: number, time: number) {
    ...
    let ix = 0, iz = 0;
    if (this.keys.has("KeyW") || this.keys.has("KeyZ") || this.keys.has("ArrowUp")) iz -= 1;
    if (this.keys.has("KeyS") || this.keys.has("ArrowDown")) iz += 1;
    if (this.keys.has("KeyA") || this.keys.has("KeyQ") || this.keys.has("ArrowLeft")) ix -= 1;
    ...
    const wantSprint =
      ((this.keys.has("ShiftLeft") || this.keys.has("ShiftRight")) || this.touchSprint || stickMag > 0.92) &&
      moving && (iz < 0 || this.touchSprint) && !this.exhausted && !this.sneaking;
