...
export class GameFX {
  ...
  constructor(
    ...
    // SSAO is too heavy for mobile GPUs — skip entirely
    if (!mobile) {
      this.ssao = new SSAOPass(scene, camera, width, height);
      this.ssao.kernelRadius = 16;
    ...
    // Skip FXAA on mobile — saves a full-screen pass
    if (!mobile) {
  ...
  setSize(width: number, height: number, pixelRatio: number) {
    ...
    if (this.ssao) this.ssao.setSize(width, height);
  }
