...
export class Entity {
  ...
  // new body part refs for the upgraded creature
  private jawGroup!: THREE.Group;
  private spineVerts: THREE.Mesh[] = [];
  private eyeMatL!: THREE.MeshStandardMaterial;
  private eyeMatR!: THREE.MeshStandardMaterial;
  ...
  private buildBody(seed: number) {
    ...
    // Bone material for skull and teeth
    const bone = new THREE.MeshStandardMaterial({
      color: 0x2a2420,
      roughness: 0.7,
      metalness: 0.05,
    });
    // Tooth material — pale, slightly translucent
    ...
    const HIP_Y = 1.55; // taller creature

    this.hips = new THREE.Group();
    this.hips.position.y = HIP_Y;
    this.root.add(this.hips);

    // ===== LEGS — longer, thinner, digitigrade feel =====
    const makeLeg = (side: 1 | -1) => {
      const leg = new THREE.Group();
      leg.position.set(0.1 * side, 0, 0);
      const thigh = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.032, 0.85, 7), skin);
      thigh.position.y = -0.425;
      const knee = new THREE.Group();
      knee.position.y = -0.85;
      const kneeCap = new THREE.Mesh(new THREE.SphereGeometry(0.042, 6, 5), skin);
      const shin = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.038, 0.78, 7), skin);
      shin.position.y = -0.39;
      const foot = new THREE.Mesh(new THREE.SphereGeometry(0.045, 6, 5), skin);
      foot.scale.set(0.7, 0.35, 2.8);
      foot.position.set(0, -0.78, 0.1);
      // Toe claws
      for (let t = 0; t < 3; t++) {
        const claw = new THREE.Mesh(new THREE.ConeGeometry(0.008, 0.06, 4), bone);
        claw.rotation.x = Math.PI * 0.6;
        claw.position.set((t - 1) * 0.025, -0.79, 0.22);
        knee.add(claw);
      }
    ...
    // ===== TORSO — gaunt, hunched, with visible spine =====
    const pelvis = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.08, 0.28, 8), skin);
    pelvis.position.y = 0.12;
    this.hips.add(pelvis);

    const torsoGeo = new THREE.CylinderGeometry(0.12, 0.085, 0.9, 8);
    torsoGeo.translate(0, 0.45, 0);
    this.torso = new THREE.Mesh(torsoGeo, skin);
    this.torso.position.y = 0.18;
    this.torso.rotation.x = 0.12;
    this.hips.add(this.torso);

    // Sunken ribs — more prominent
    for (let i = 0; i < 5; i++) {
      const rib = new THREE.Mesh(
        new THREE.TorusGeometry(0.1 - i * 0.006, 0.006, 5, 12, Math.PI),
        skin,
      );
      rib.rotation.x = Math.PI / 2 + 0.12;
      rib.rotation.z = Math.PI;
      rib.position.set(0, 0.3 - i * 0.065, 0.04);
      this.torso.add(rib);
    }

    // ===== SPINE RIDGES on the back =====
    this.spineVerts = [];
    for (let i = 0; i < 6; i++) {
      ...
      vert.scale.set(1.4, 1.0, 2.0);
      vert.position.set(0, 0.15 + i * 0.13, -0.09);
      this.torso.add(vert);
      this.spineVerts.push(vert);
    }

    // ===== ARMS — longer, thinner, with skeletal clawed hands =====
    const SHOULDER_Y = 0.82;
    const makeArm = (side: 1 | -1) => {
      const arm = new THREE.Group();
      arm.position.set(0.2 * side, SHOULDER_Y, 0);
      const shoulder = new THREE.Mesh(new THREE.SphereGeometry(0.048, 6, 5), skin);
      const upper = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.02, 0.75, 6), skin);
      upper.position.y = -0.375;
      const elbow = new THREE.Group();
      elbow.position.y = -0.75;
      const elbowCap = new THREE.Mesh(new THREE.SphereGeometry(0.028, 6, 5), skin);
      const fore = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.024, 0.7, 6), skin);
      fore.position.y = -0.35;
      const palm = new THREE.Mesh(new THREE.SphereGeometry(0.035, 6, 5), skin);
      palm.scale.set(0.9, 1.6, 0.55);
      palm.position.y = -0.72;
      // 5 fingers, each with 3 segments — skeletal claws
      const fingers: THREE.Group[] = [];
      for (let fi = 0; fi < 5; fi++) {
        const finger = new THREE.Group();
        finger.position.set((fi - 2) * 0.018 * side, -0.76, 0.005);
        finger.rotation.z = (fi - 2) * 0.08 * side;

        let parent: THREE.Object3D = finger;
        for (let seg = 0; seg < 3; seg++) {
          const segLen = 0.07 - seg * 0.008;
          const segR = 0.006 - seg * 0.001;
          const segMesh = new THREE.Mesh(
            new THREE.CapsuleGeometry(segR, segLen, 3, 5),
            skin,
          );
        ...
        // Claw tip
        const claw = new THREE.Mesh(new THREE.ConeGeometry(0.005, 0.03, 4), bone);
    ...
    const LA = makeArm(-1);
    const RA = makeArm(1);
    this.armL = LA.arm; this.elbowL = LA.elbow; this.fingersL = LA.fingers;
    this.armR = RA.arm; this.elbowR = RA.elbow; this.fingersR = RA.fingers;

    // ===== NECK — thin, elongated =====
    const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.04, 0.22, 7), skin);
    neck.position.set(0, 0.92, 0.015);
    this.torso.add(neck);

    // ===== HEAD — elongated skull with bone ridges =====
    this.headGroup = new THREE.Group();
    this.headGroup.position.set(0, 1.04, 0.025);

    const skullProfile: THREE.Vector2[] = [
      new THREE.Vector2(0.001, 0),
      new THREE.Vector2(0.055, 0.015),
      new THREE.Vector2(0.072, 0.06),
      new THREE.Vector2(0.058, 0.12),
      new THREE.Vector2(0.085, 0.19),
      new THREE.Vector2(0.105, 0.26),
      new THREE.Vector2(0.115, 0.34),
      new THREE.Vector2(0.125, 0.42),
      new THREE.Vector2(0.1, 0.50),
      new THREE.Vector2(0.055, 0.55),
      new THREE.Vector2(0.001, 0.57),
    ];
    const headGeo = new THREE.LatheGeometry(skullProfile, 16);
    headGeo.scale(0.92, 1, 0.85);
    const head = new THREE.Mesh(headGeo, bone);
    head.position.y = -0.06;
    this.headGroup.add(head);

    // Bone ridges on cranium
    for (let i = 0; i < 3; i++) {
      ...
      ridge.position.set(0, 0.38 + i * 0.05, 0.06);
      this.headGroup.add(ridge);
    }

    // ===== EYES — large, bright glowing white orbs =====
    this.eyeMatL = new THREE.MeshStandardMaterial({
      color: 0xffffff,
      emissive: 0xffffff,
    ...
    this.eyeMatR = this.eyeMatL.clone();

    const eyeGeo = new THREE.SphereGeometry(0.028, 10, 10);
    const eyeL = new THREE.Mesh(eyeGeo, this.eyeMatL);
    eyeL.position.set(-0.045, 0.2, 0.072);
    const eyeR = new THREE.Mesh(eyeGeo.clone(), this.eyeMatR);
    eyeR.position.set(0.045, 0.2, 0.072);
    this.headGroup.add(eyeL, eyeR);

    // Eye socket hollows
    const socketMat = new THREE.MeshStandardMaterial({ color: 0x050505, roughness: 0.95 });
    const socketGeo = new THREE.SphereGeometry(0.038, 8, 8);
    const socketL = new THREE.Mesh(socketGeo, socketMat);
    socketL.position.set(-0.045, 0.2, 0.065);
    socketL.scale.set(1, 1, 0.5);
    const socketR = new THREE.Mesh(socketGeo.clone(), socketMat);
    socketR.position.set(0.045, 0.2, 0.065);
    socketR.scale.set(1, 1, 0.5);
    this.headGroup.add(socketL, socketR);

    // ===== MOUTH — wide grinning jaw with teeth =====
    // Upper jaw (fixed to skull)
    const upperJaw = new THREE.Group();
    upperJaw.position.set(0, 0.08, 0.04);
    for (let t = 0; t < 9; t++) {
      const angle = ((t - 4) / 4.5) * 0.65;
      const tx = Math.sin(angle) * 0.055;
      const tz = Math.cos(angle) * 0.035;
      const toothH = 0.018 + Math.random() * 0.012;
      const tooth = new THREE.Mesh(new THREE.ConeGeometry(0.004, toothH, 4), toothMat);
      tooth.position.set(tx, -toothH / 2 - 0.003, tz);
      tooth.rotation.x = 0.1;
      upperJaw.add(tooth);
    }
    this.headGroup.add(upperJaw);

    // Lower jaw (animated)
    this.jawGroup = new THREE.Group();
    this.jawGroup.position.set(0, 0.05, 0.02);
    const jawBone = new THREE.Mesh(new THREE.BoxGeometry(0.09, 0.02, 0.06), bone);
    jawBone.position.y = -0.01;
    this.jawGroup.add(jawBone);
    for (let t = 0; t < 9; t++) {
      const angle = ((t - 4) / 4.5) * 0.65;
      const tx = Math.sin(angle) * 0.052;
      const tz = Math.cos(angle) * 0.033;
      const toothH = 0.015 + Math.random() * 0.01;
      const tooth = new THREE.Mesh(new THREE.ConeGeometry(0.0035, toothH, 4), toothMat);
      tooth.position.set(tx, toothH / 2 + 0.003, tz);
    ...
    // Scale the creature down to ~2.1m so it easily fits under the ceiling and doors
  ...
  debugSpawn(playerPos: THREE.Vector3, camDir: THREE.Vector3) {
    this.root.visible = true;
    this.state = "stalk";
    this.path = [];
    this.waypoint = null;
    
    // Spawn slightly in front of the player (1.5m) so it's easily visible
    const dir = new THREE.Vector3(camDir.x, 0, camDir.z).normalize();
    this.pos.copy(playerPos).addScaledVector(dir, 1.5);
    
    // Safety check: if the spawn position is blocked (inside a wall), just spawn exactly at player's position
    const cell = this.level.cellOf(this.pos.x, this.pos.z);
    if (this.level.isBlocked(cell.x, cell.z)) {
      this.pos.copy(playerPos);
    }
    
    // Force the entity to always stay on the floor!
    this.pos.y = 0;
  ...
  private animate(dt: number, ctx: EntityContext, speed: number, dist: number) {
    ...
    this.hips.position.y = 1.55 + (this.frozen ? 0 : Math.abs(Math.sin(this.walkPhase)) * 0.05)
      - (this.state === "chase" ? 0.09 : 0);
    this.torso.rotation.x = 0.12 + (this.state === "chase" ? 0.35 : 0);

    // Spine flex — writhing organic movement on the back ridges
    this.spineVerts.forEach((vert, i) => {
      vert.position.x = Math.sin(ctx.time * 3 + i * 0.5) * 0.01;
      vert.rotation.y = Math.cos(ctx.time * 2 + i * 0.4) * 0.1;
    });

    // Jaw Gape — slightly open when roaming, gapes terrifyingly wide when chasing
    const jawIdle = 0.05 + Math.max(0, Math.sin(this.walkPhase * 2)) * 0.05;
    const jawChase = 0.4 + Math.sin(ctx.time * 20) * 0.05;
    const jawTarget = this.state === "chase" ? jawChase : jawIdle;
    this.jawGroup.rotation.x += (jawTarget - this.jawGroup.rotation.x) * k;

    // Glowing Eyes Pulse — mimics a frantic heartbeat
    const heartBeat = Math.pow(Math.sin(ctx.time * (this.state === "chase" ? 12 : 3)), 8);
    const eyeBase = this.state === "chase" ? 3.5 : 1.5;
    this.eyeMatL.emissiveIntensity = eyeBase + heartBeat * 1.5;
    this.eyeMatR.emissiveIntensity = eyeBase + heartBeat * 1.5;
    ...
    if (this.twitchT > 0) {
      ...
      this.headGroup.rotation.x += this.twitchVec.x * t * 0.45;
      this.headGroup.rotation.y += this.twitchVec.y * t * 0.45;
      this.headGroup.rotation.z = this.twitchVec.z * t * 0.4;
      this.torso.rotation.z = this.twitchVec.z * t * 0.18;
    } else {
