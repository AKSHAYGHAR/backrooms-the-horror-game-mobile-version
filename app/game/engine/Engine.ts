...
import { PennywiseEntity } from "./pennywise";
...
export interface HudState {
  ...
  mapCheat: boolean;
  /** 0.0 when safe, approaching 1.0 as the monster gets very close */
  dangerLevel: number;
}
...
export class Engine {
  ...
  private entity: Entity | PennywiseEntity;
  private items: Items;
  ...
  constructor(
    ...
    // Mobile: cap pixel ratio to 1.25 for buttery 60fps without GPU heat/battery drain; desktop up to 2
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, this.touchPrimary ? 1.25 : 2));
    // Disable shadows entirely on mobile — too heavy for phone GPUs
    this.renderer.shadowMap.enabled = !this.touchPrimary;
    ...
    // Reduced ambient lighting for higher contrast and tension
    this.scene.add(new THREE.AmbientLight(0x3a3420, this.touchPrimary ? 0.55 : 0.35));
    this.scene.add(new THREE.HemisphereLight(0xfff0c2, 0x4a3f24, this.touchPrimary ? 0.35 : 0.25));

    this.level = new Level(seed, this.levelIndex);
    ...
    // Create entity based on level
    if (this.levelIndex === 1) {
      this.entity = new PennywiseEntity(this.level, seed);
    } else {
    ...
    // Fewer dynamic lights on mobile
    const poolSize = this.touchPrimary ? 6 : POOL_SIZE;
    for (let i = 0; i < poolSize; i++) {
      const l = new THREE.PointLight(0xffeebb, 0, 13, 1.8);
      // We cannot enable shadows on 12 point lights, it exceeds 16 texture units in WebGL
      this.lightPool.push(l);
      this.scene.add(l);
    }

    this.fx = new GameFX(this.renderer, this.scene, this.player.camera, width, height, this.touchPrimary);
  ...
  setSettings(s: { volume: number; quality: string; sensitivity: number; invertY: boolean }) {
    this.audio.setVolume(s.volume);
    this.player.mouseSens = s.sensitivity;
    this.player.invertY = s.invertY;
    if (s.quality === "low") {
      this.renderer.shadowMap.enabled = false;
      this.scene.fog = new THREE.FogExp2(0x141106, 0.045);
      this.renderer.setPixelRatio(1.0);
    } else if (s.quality === "normal" || (this.touchPrimary && s.quality !== "high")) {
      // Normal/balanced setting: ideal for mobile devices & smooth framerates
      this.renderer.shadowMap.enabled = !this.touchPrimary;
      this.scene.fog = new THREE.FogExp2(0x141106, 0.038);
      this.renderer.setPixelRatio(
        this.touchPrimary ? Math.min(window.devicePixelRatio, 1.25) : Math.min(window.devicePixelRatio, 1.75)
      );
    } else {
      // High fidelity (PC default)
      this.renderer.shadowMap.enabled = !this.touchPrimary;
      this.scene.fog = new THREE.FogExp2(0x141106, 0.036);
      this.renderer.setPixelRatio(
        this.touchPrimary ? Math.min(window.devicePixelRatio, 1.5) : Math.min(window.devicePixelRatio, 2)
      );
    }
  ...
  private attachInput() {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyP" && !this.cheats.unlocked) {
  ...
  private loop = () => {
    ...
    if (this.state === "playing") {
      ...
      const exitPos = this.level.exit.doorPos;
      const d = Math.hypot(this.player.pos.x - exitPos.x, this.player.pos.z - exitPos.z);
      if (this.items.exitOpen && d < 1.6) {
    ...
    for (const f of fixtures) {
      ...
      if (entityActive) {
        ...
        if (this.dangerLevel > 0) {
          // If dangerLevel is high, lights in a wide radius are smothered completely.
          // dangerLevel=0 -> normal aura (8m). dangerLevel=1 -> huge aura (20m)
          const radius = 8 + this.dangerLevel * 12;
          if (dEntSq < radius * radius) {
            const aura = 1 - Math.sqrt(dEntSq) / radius;
            f.aura += (aura - f.aura) * Math.min(1, dt * 6);
          } else {
            f.aura += (0 - f.aura) * Math.min(1, dt * 3);
          }
          if (f.aura > 0.01) {
            // As danger goes up, the smothering becomes absolute (blackout).
            const strangleForce = Math.max(0.75, this.dangerLevel * 1.5);
            const strangle = Math.random() < f.aura * 0.7 ? 0.06 : Math.max(0, 1 - f.aura * strangleForce);
            mult *= strangle;
          }
        } else {
          // Normal logic for Wanderer / low danger
          if (dEntSq < 64) {
  ...
  private updateFearAndAudio(dt: number) {
    ...
    // Update danger level based on distance to entity
    if (this.entity.state !== "dormant") {
      const dist = this.entity.pos.distanceTo(playerPos);
      // Danger scales from 0 at 22m to 1 at 3m
      this.dangerLevel = Math.max(0, Math.min(1, 1 - (dist - 3) / 19));
    } else {
      this.dangerLevel = 0;
    }
