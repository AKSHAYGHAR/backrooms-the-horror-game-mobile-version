...
import {
  ...
  makeWallArtTexture,
  makeWallMaps,
  makeConcreteWallMaps,
  makeConcreteFloorMaps,
} from "./textures";
...
export class Level {
  ...
  constructor(public seed: number, public levelIndex: number = 0) {
  ...
  private generate() {
    ...
    const divide = (x0: number, z0: number, x1: number, z1: number, depth: number) => {
      ...
      const leaveThreshold = this.levelIndex === 1 ? 80 : 30;
      const leaveProb = this.levelIndex === 1 ? 0.6 : 0.3;
      if (w * h <= leaveThreshold && rng() < leaveProb && depth > 2) return;
  ...
  build(scene: THREE.Scene) {
    ...
    const wall = this.levelIndex === 1 ? makeConcreteWallMaps(seed) : makeWallMaps(seed);
    const carpet = this.levelIndex === 1 ? makeConcreteFloorMaps(seed) : makeCarpetMaps(seed);
    ...
    const wallMat = new THREE.MeshStandardMaterial({
      ...
      normalScale: new THREE.Vector2(1.8, 1.8), // Enhanced for realistic bump depth
      metalness: 0.15,
    });
