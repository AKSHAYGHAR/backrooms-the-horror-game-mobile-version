...
export default function GameCanvas({ callbacksRef, onReady, levelIndex = 0 }: Props) {
    ...
    const raf = setTimeout(() => {
      try {
        engine = new Engine(container, canvas, {
          ...
          onNextLevel: () => callbacksRef.current?.onNextLevel(),
        }, levelIndex);
        ...
        callbacksRef.current?.onToast("Engine Error: " + e.message);
